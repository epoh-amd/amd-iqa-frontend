...
const EditDataPage = () => {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      const data = await api.getProjects(); // create this API if not yet
  ...
  const addProject = async (projectName) => {
    ...
    try {
      const res = await api.addProject({ project_name: trimmed });
  
      setProjectOptions(prev => [...prev, trimmed]);
  ...
  const handleSaveAll = async () => {
    if (Object.keys(editedRows).length === 0) {
      setMessages([
        { type: 'success', text: 'No changes updated.' }
      ]);
      return;
    }
  
    try {
      console.log("editted rows", editedRows);
      await api.bulkUpdateBuilds(editedRows);
